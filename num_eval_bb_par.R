...
foreach (trueR = 1:length(latentRseq)) %:%
  foreach (zrate = 1:length(zratioseq), .combine = rbind) %dopar% {
    ...
    for(i in 1:nrep){
      ...
      estR(X = cbind(x1, x2), types = c("bin", "bin"), method = "original")$R[1, 2], times = 5)$time) / 10^6
      time_ml[i] <- median(microbenchmark::microbenchmark(Kcor_ml[i] <-
      estR(X = cbind(x1, x2), types = c("bin", "bin"), ratio = 1)$R[1, 2], times = 5)$time) / 10^6
      time_mlbd[i] <- median(microbenchmark::microbenchmark(Kcor_mlbd[i] <-
      estR(X = cbind(x1, x2), types = c("bin", "bin"), ratio = 0.9)$R[1, 2], times= 5)$time) /10^6
    }
