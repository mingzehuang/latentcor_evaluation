...
foreach (trueR = 1:length(latentRseq), .combine = rbind) %:%
  foreach (zrate = 1:length(zratioseq), .combine = rbind) %dopar% {
    ...
    TB_eval <- data.frame(LatentR = latentRseq[trueR], TruncRate = zratioseq[zrate], MeanAD = c(0, mean(abs(Kcor_org - Kcor_ml)), mean(abs(Kcor_org - Kcor_mlbd))), MaxAD = c(0, max(abs(Kcor_org - Kcor_ml)), max(abs(Kcor_org - Kcor_mlbd))), medianTime = apply(time_all, 2, summary)[3, ], method = c("org", "ipol", "ipol_UB"))
  }
save(TB_eval, file = "TB_eval.rda")