...
NN_eval <-
  foreach (trueR = 1:length(latentRseq)) %:%
    foreach (zrate = 1:length(zratioseq), .combine = rbind) %dopar% {
      ...
      latentcor(X = cbind(x1, x2), types = c("ter", "ter"), method = "original")$R[1, 2], times = 5)$time) / 10^6
      time_ml[i] <- median(microbenchmark::microbenchmark(Kcor_ml[i] <-
      latentcor(X = cbind(x1, x2), types = c("ter", "ter"), ratio = 1)$R[1, 2], times = 5)$time) / 10^6
      time_mlbd[i] <- median(microbenchmark::microbenchmark(Kcor_mlbd[i] <-
      latentcor(X = cbind(x1, x2), types = c("ter", "ter"), ratio = 0.9)$R[1, 2], times= 5)$time) / 10^6
    }
...
save(NN_eval_3d, file = "NN_eval.rda", compress = "xz")
