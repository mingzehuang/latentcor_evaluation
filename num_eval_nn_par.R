...
foreach (trueR = 1:length(latentRseq)) %:%
  foreach (zrate11 = 1:length(zratioseq1)) %dopar% {
    df <- NULL
    zratioseq12 = seq(zratioseq1[zrate11] + 0.04, 0.96, by = 0.04)
    for (zrate21 in 1:length(zratioseq1)){
      for (zrate12 in 1:length(zratioseq12)){
        zratioseq22 = seq(zratioseq1[zrate21] + 0.04, 0.96, by = 0.04)
        for (zrate22 in 1:length(zratioseq22)){
        ...
        df <- rbind.data.frame(df, data.frame(LatentR = latentRseq[trueR], TruncRate11 = zratioseq1[zrate11], TruncRate12 = zratioseq12[zrate12],
                                              TruncRate21 = zratioseq1[zrate21], TruncRate22 = zratioseq22[zrate22],
                                              MeanAD = c(0, mean(abs(Kcor_org - Kcor_ml)), mean(abs(Kcor_org - Kcor_mlbd))),
                                              MaxAD = c(0, max(abs(Kcor_org - Kcor_ml)), max(abs(Kcor_org - Kcor_mlbd))),
                                              medianTime = apply(time_all, 2, summary)[3, ],
                                              method = c("org", "ipol", "ipol_UB")))
      }
    }
    NN_eval <- df
  }
