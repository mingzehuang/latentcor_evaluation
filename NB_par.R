...
d11_grid <- d12_grid <- d2_grid <- round(pnorm(seq(-1.8, 1.8, by =.3), sd = .8), 6)
  ...
  foreach (j = 1:l_d11_grid) %dopar% {
    ...
    for (k in 1:l_d12_grid) {
      for (l in 1:l_d2_grid) {
        ...
        tau = tau_grid[i] * bound_nb(zratio1 = zratio1, zratio2 = zratio2)
        value[k, l] = r_sol(type1 = "ternary", type2 = "binary", tau = tau, zratio1 = zratio1, zratio2 = zratio2, tol = 1e-6)  
      }
    }
    value_list <- value
  }
...
save(NBvalue, NBipol, file = "NB_grid.rda", compress = "xz")
